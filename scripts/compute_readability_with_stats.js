...
const wordsArg = Number(process.argv[3]);
const sentencesArg = Number(process.argv[4]);
if (!file || !wordsArg || !sentencesArg) { console.error('Usage: node compute_readability_with_stats.js <file> <words> <sentences>'); process.exit(2);} 
const text = fs.readFileSync(file, 'utf8');
const words = wordsArg;
const sentences = sentencesArg;
...
console.log({ words, sentences, syllables, complexWords, polysyllables, characters });
console.log({ fleschKincaid: Number(fleschKincaid.toFixed(1)), fleschReadingEase: Number(fleschReadingEase.toFixed(1)), gunningFog: Number(gunningFog.toFixed(1)), smog: Number(smog.toFixed(1)), colemanLiau: Number(colemanLiau.toFixed(1)), automatedReadability: Number(automatedReadability.toFixed(1)), daleChall: Number(daleChall.toFixed(1)) });
